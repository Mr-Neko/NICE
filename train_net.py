...
def train_epoch(train_loader, bert_encoder, fpn_model, model, 
    ...
    for (batch_idx, (caption, grounding_instances, ann_categories, \
        ...
        _, _, predictions, predictions_box = model(fpn_feature, lang_feat_valid, ann_types) #Knet
        ...
        with torch.no_grad():
            ...
            gts_box = [grounding_instances[i]["gt_box"].to(cfg.local_rank) for i in range(len(grounding_instances))]
            gts_box = torch.stack(gts_box, dim=0)

        loss = 0
        grad_sample = ann_types != 0
        gt = gts[grad_sample]
        gt_box = gts_box[grad_sample]
        ...
        pred_box = predictions_box[-1][grad_sample]
        loss += l1_loss(pred_box, gt_box).mean() + giou_loss(pred_box, gt_box).mean()
...
def evaluate(val_loader, bert_encoder, fpn_model, model, epoch, cfg, logger, writer):
    ...
    for (batch_idx, (caption, grounding_instances, ann_categories, \
        ann_types, noun_vector_padding, ret_noun_vector, fpn_input_data, ann_types_val)) in enumerate(val_loader):
        ann_categories = ann_categories.to(cfg.local_rank)
        ann_types = ann_types.to(cfg.local_rank)
        # ret_noun_vector = ret_noun_vector.to(cfg.local_rank)
        ...
        with torch.no_grad():
            ...
            _, _, predictions, predictions_boxes = model(fpn_feature, lang_feat_valid, ann_types_val)
            end_time = time.time()
            ...
            predictions_boxes = predictions_valid_boxes
        ...
        # Gather all the predictions across all the devices.
        if cfg.num_gpus > 1:
            predictions, predictions_boxes, gts, gts_box, ann_categories, ann_types = all_gather(
                [predictions, predictions_boxes, gts, gts_box, ann_categories, ann_types]
            )

        # Evaluation

        for p, pb, t, tb, th, s in zip(predictions, predictions_boxes, gts, gts_box, ann_categories, ann_types):
            for i in range(cfg.max_phrase_num):
                if s[i] == 0:
                    ...
                    if s[i] == 1:
                        ...
                        box_singulars_iou.append(box_iou.cpu().item())
                    else:
                        plurals_iou.append(instance_iou.cpu().item())
                        box_plurals_iou.append(box_iou.cpu().item())
                    if th[i] == 1:
                        things_iou.append(instance_iou.cpu().item())
                        box_things_iou.append(box_iou.cpu().item())
                    else:
                        stuff_iou.append(instance_iou.cpu().item())
                        box_stuff_iou.append(box_iou.cpu().item())
    ...
    box_AA_singulars = average_accuracy(box_singulars_iou, save_fig=cfg.save_fig, output_dir=cfg.output_dir, filename='box_singulars')
    box_AA_plurals = average_accuracy(box_plurals_iou, save_fig=cfg.save_fig, output_dir=cfg.output_dir, filename='box_plurals')
    box_AA_things = average_accuracy(box_things_iou, save_fig=cfg.save_fig, output_dir=cfg.output_dir, filename='box_things')
    box_AA_stuff = average_accuracy(box_stuff_iou, save_fig=cfg.save_fig, output_dir=cfg.output_dir, filename='box_stuff')
    ...
    if dist.get_rank()==0:
        ...
        logger.info('| final box_acc@0.5: {:.5f} | final box_AA: {:.5f} |  box_AA singulars: {:.5f} | box_AA plurals: {:.5f} | box_AA things: {:.5f} | box_AA stuff: {:.5f} |'.format(
                                            box_accuracy,
                                            box_AA,
                                            box_AA_singulars,
                                            box_AA_plurals,
        ...
        writer.add_scalar('aa/box_singulars', box_AA_singulars, epoch)
        writer.add_scalar('aa/box_plurals', box_AA_plurals, epoch)
        writer.add_scalar('aa/box_things', box_AA_things, epoch)
        writer.add_scalar('aa/box_stuffs', box_AA_stuff, epoch)
