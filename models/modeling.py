...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None):
        ...
        words_embeddings = self.word_embeddings(input_ids) #[60,230,768]
        position_embeddings = self.position_embeddings(position_ids) #[60,230,768]
        token_type_embeddings = self.token_type_embeddings(token_type_ids) #[60,230,768]
...
class VisualFeatEncoder(nn.Module):
    ...
    def forward(self, visn_input):
        ...
        x = self.visn_layer_norm(x) #[60,58,768]
        y = self.box_fc(boxes)
        y = self.box_layer_norm(y) #[60,58,768]
        output = (x + y) / 2
...
class CrossEncoder(nn.Module):
    ...
    def forward(self, lang_feats, lang_attention_mask):
        ...
        # visn_feats = self.visn_fc(visn_feats) #[60,58,768]
        for layer_module in self.layer:
            lang_feats = layer_module(lang_feats, lang_attention_mask) #[60,230,768]

        # for layer_module in self.single_layers:
        #     visn_feats = layer_module(visn_feats, visn_attention_mask) #[60,56,768]
